...
class AutoClicker:
    def __init__(self, root):
        self.root = root
        self.root.title("AutoClicker Pro v3")
        ...
        self.toggle_key = Key.f6 if PYNPUT_AVAILABLE else None
        self.toggle_key_name = "F6"
        self.listener = None
        self.capturing_key = False
        self.click_mode = "xdotool"
    ...
    def setup_ui(self):
        title = tk.Label(self.root, text="AutoClicker Pro v3", font=("Segoe UI", 18, "bold"), 
                        bg="#1e1e2e", fg="#89b4fa")
        title.pack(pady=10)
        ...
        mode_frame = tk.LabelFrame(main_frame, text=" Modo de Clique", bg="#313244", 
                                   fg="#cdd6f4", font=("Segoe UI", 11, "bold"), padx=15, pady=8)
        mode_frame.pack(fill="x", pady=5)
        
        self.click_mode_var = tk.StringVar(value="xdotool")
        tk.Radiobutton(mode_frame, text="Xdotool (Recomendado - funciona em qualquer lugar)", 
                       variable=self.click_mode_var, value="xdotool",
                       bg="#313244", fg="#cdd6f4", selectcolor="#1e1e2e",
                       activebackground="#313244", activeforeground="#cdd6f4",
                       font=("Segoe UI", 10), command=self.update_click_mode).pack(anchor="w", pady=2)
        tk.Radiobutton(mode_frame, text="Pynput (Pode nao funcionar fora da janela)", 
                       variable=self.click_mode_var, value="pynput",
                       bg="#313244", fg="#cdd6f4", selectcolor="#1e1e2e",
                       activebackground="#313244", activeforeground="#cdd6f4",
                       font=("Segoe UI", 10), command=self.update_click_mode).pack(anchor="w", pady=2)
        
        shortcut_frame = tk.LabelFrame(main_frame, text=" Tecla de Atalho", bg="#313244", 
                                       fg="#cdd6f4", font=("Segoe UI", 11, "bold"), padx=15, pady=8)
        ...
        if PYNPUT_AVAILABLE:
            ...
            tk.Label(shortcut_frame, text="(pynput nao instalado - use botao)", 
                     bg="#313244", fg="#f38ba8", font=("Segoe UI", 9)).pack()
        ...
        mode2_frame = tk.LabelFrame(main_frame, text=" Modo", bg="#313244", fg="#cdd6f4",
                                    font=("Segoe UI", 11, "bold"), padx=15, pady=8)
        mode2_frame.pack(fill="x", pady=5)
        
        self.mode_var = tk.StringVar(value="single")
        tk.Radiobutton(mode2_frame, text="Simples", variable=self.mode_var, 
                       value="single", bg="#313244", fg="#cdd6f4", selectcolor="#1e1e2e",
                       activebackground="#313244", activeforeground="#cdd6f4",
                       font=("Segoe UI", 10)).grid(row=0, column=0, padx=5)
        tk.Radiobutton(mode2_frame, text="Duplo", variable=self.mode_var, 
        ...
        self.toggle_btn = tk.Button(btn_frame, text=" INICIAR", command=self.toggle_clicking,
    ...
    def start_key_capture(self):
        if not PYNPUT_AVAILABLE or self.capturing_key:
            return
    ...
    def update_key_display(self):
        ...
        self.toggle_btn.config(text=f" INICIAR ({self.toggle_key_name})")
    ...
    def get_key_name(self, key):
        if hasattr(key, 'char') and key.char:
            ...
            key_names = {
                ...
                Key.space: "ESPACO", Key.enter: "ENTER", Key.esc: "ESC",
                ...
                Key.up: "CIMA", Key.down: "BAIXO", Key.left: "ESQ", Key.right: "DIR",
    ...
    def update_click_button(self):
        self.click_button = self.click_type_var.get()
    ...
    def start_clicking(self):
        ...
        self.toggle_btn.config(text=f" PARAR ({self.toggle_key_name})", bg="#f38ba8")
    ...
    def stop_clicking(self):
        ...
        self.toggle_btn.config(text=f" INICIAR ({self.toggle_key_name})", bg="#a6e3a1")
    
    def click_loop(self):
        while self.clicking:
            if self.click_mode == "xdotool":
                self.click_with_xdotool()
            else:
                self.click_with_pynput()
            
            self.root.after(0, self.update_count_label)
            time.sleep(self.delay)
    
    def click_with_xdotool(self):
        try:
            btn = self.click_button
            count = 2 if self.mode_var.get() == "double" else 1
            ...
            print(f"Erro xdotool: {e}")
    
    def click_with_pynput(self):
        try:
            mouse = MouseController()
            btn_map = {"left": Button.left, "right": Button.right, "middle": Button.middle}
            btn = btn_map.get(self.click_button, Button.left)
